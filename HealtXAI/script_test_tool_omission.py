...
# Verifica che la colonna tool_omission_number esista in tracked_anomalies
query_check_tool_omission_column = """SELECT EXISTS (
    SELECT 1
    FROM information_schema.columns
    WHERE table_name = 'tracked_anomalies'
      AND column_name = 'tool_omission_number'
) AS column_exists;"""

query_add_tool_omission_column = '''ALTER TABLE tracked_anomalies
ADD COLUMN tool_omission_number SMALLINT;'''

col_check = take_data(query_check_tool_omission_column)
if col_check and not col_check[0]["column_exists"]:
    print("debug : aggiungiamo la colonna tool_omission_number a tracked_anomalies")
    insert_data(query_add_tool_omission_column)
else:
    print("debug : colonna tool_omission_number gia' presente in tracked_anomalies")

# Scrive i risultati nel database con upsert
for (patient_id, activity_id), tool_omission_count in sorted(
    patient_anomalies.items(),
    key=lambda item: (int(item[0][0]), int(item[0][1])),
):
    query_upsert = f'''INSERT INTO tracked_anomalies
    (patient_id, activity_id, tool_omission_number)
    VALUES ({patient_id}, {activity_id}, {tool_omission_count})
    ON CONFLICT (patient_id, activity_id)
    DO UPDATE SET tool_omission_number = EXCLUDED.tool_omission_number'''

    try:
        insert_data(query_upsert)
        print(
            f"debug : salvato nel DB tool_omission per patient_id={patient_id}, "
            f"activity_id={activity_id} -> {tool_omission_count}"
        )
    except Exception as e:
        print(
            f"errore : salvataggio DB tool_omission per patient_id={patient_id}, "
            f"activity_id={activity_id}: {e}"
        )

print("debug : salvataggio tool omission nel database completato")
