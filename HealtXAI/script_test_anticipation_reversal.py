...
# Verifica che le colonne reversal_number e anticipation_omission_number esistano
def ensure_sequence_columns():
    columns_to_check = [
        ("reversal_number", "SMALLINT"),
        ("anticipation_omission_number", "SMALLINT"),
    ]
    for col_name, col_type in columns_to_check:
        ...
        col_check = take_data(query_check)
        if col_check and not col_check[0]["column_exists"]:
            print(f"debug : aggiungiamo la colonna {col_name} a tracked_anomalies")
            insert_data(f"ALTER TABLE tracked_anomalies ADD COLUMN {col_name} {col_type};")
        else:
            print(f"debug : colonna {col_name} gia' presente in tracked_anomalies")

ensure_sequence_columns()

# Scrive i risultati nel database con upsert
for (patient_id, activity_id), anomaly_info in sorted(
    patient_anomalies.items(),
    key=lambda item: (int(item[0][0]), int(item[0][1])),
):
    reversal_count = len(anomaly_info["reversal"])
    anticipation_count = len(anomaly_info["anticipation_omission"])

    query_upsert = f'''INSERT INTO tracked_anomalies
    (patient_id, activity_id, reversal_number, anticipation_omission_number)
    VALUES ({patient_id}, {activity_id}, {reversal_count}, {anticipation_count})
    ON CONFLICT (patient_id, activity_id)
    DO UPDATE SET reversal_number = EXCLUDED.reversal_number,
                  anticipation_omission_number = EXCLUDED.anticipation_omission_number'''

    try:
        insert_data(query_upsert)
        print(
            f"debug : salvato nel DB per patient_id={patient_id}, "
            f"activity_id={activity_id} -> reversal={reversal_count}, "
            f"anticipation_omission={anticipation_count}"
        )
    except Exception as e:
        print(
            f"errore : salvataggio DB per patient_id={patient_id}, "
            f"activity_id={activity_id}: {e}"
        )

print("debug : salvataggio anticipation/reversal nel database completato")
